...
function modalFields(modal: AdvancedAction, detail: Detail, operations: Array<{ _id: Id<"rndcOperations">; documentId?: Id<"documents">; status: string }>, eligible: Detail["documents"], remesas: Detail["documents"], manifests: Detail["documents"]) {
  ...
  return <><div className="modal-info wide">Esta remesa no heredará datos de una orden de cargue. Todos los datos operativos y RNDC son obligatorios.</div><label><span>Remitente</span><input name="senderName" required /></label><label><span>Identificación remitente</span><input name="senderId" required /></label><label><span>Sede remitente</span><input name="senderSiteCode" required /></label><label><span>Destinatario</span><input name="recipientName" required /></label><label><span>Identificación destinatario</span><input name="recipientId" required /></label><label><span>Sede destinatario</span><input name="recipientSiteCode" required /></label><label><span>Dirección de cargue</span><input name="loadingAddress" required /></label><ModalMunicipalityField cityInputName="loadingCity" label="Municipio de cargue" municipalityInputName="loadingMunicipality" /><label><span>Cita de cargue</span><input name="loadingAppointment" required type="datetime-local" /></label><label><span>Dirección de descargue</span><input name="unloadingAddress" required /></label><ModalMunicipalityField cityInputName="unloadingCity" label="Municipio de descargue" municipalityInputName="unloadingMunicipality" /><label><span>Cita de descargue</span><input name="unloadingAppointment" required type="datetime-local" /></label><label><span>Mercancía</span><input name="cargoDescription" required /></label><label><span>Código mercancía</span><input name="merchandiseCode" required /></label><label><span>Cantidad</span><input name="quantity" required /></label><label><span>Peso (TN)</span><input name="weightTons" required /></label><label><span>Código empaque</span><input name="packagingCode" required /></label><label><span>Naturaleza de carga</span><input defaultValue="1" name="natureOfCargo" required /></label><label><span>Valor declarado</span><input name="declaredValue" required /></label><label><span>Número de póliza</span><input name="policyNumber" required /></label><label><span>Vencimiento póliza</span><input name="policyExpiresOn" required type="date" /></label><label><span>NIT aseguradora</span><input name="insurerNit" required /></label></>;
}

function ModalMunicipalityField({ label, municipalityInputName, cityInputName }: { label: string; municipalityInputName: string; cityInputName?: string }) {
  const [division, setDivision] = useState<DivisionPick | null>(null);
  return (
    <>
      <MunicipalityField code={division?.code} label={label} name={municipalityInputName} onClear={() => setDivision(null)} onSelect={setDivision} required />
      {cityInputName ? <input name={cityInputName} type="hidden" value={division ? (division.isMunicipality ? division.name : division.municipalityName) : ""} /> : null}
    </>
  );
}
